...
### 회복(Recovery) : 교착상태 발생을 아예 막지않고, 만약, 교착상태가 발생하면 발생이후에 문제를 해결하는 방법
      ...
      2) 복귀(Rollback)를 통한 회복 : 교착상태가 발생하기 이전의 상태로 복귀하여 다시 실행하는 방법
      3) 제거(Kill)를 통한 회복 : 우선순위가 낮은 프로세스 / 처리된 진행상태가 적은 프로세스 / 
                                 자원을 적게 사용하고 있는 프로세스 / 기아상태나 문제가 있는 프로세스 /
                                 정상수행이 불가능한 모든 프로세스를 선택하여 제거하고 다시 시작함
      4) 사용자의 조치경로 선택(Routing) : window는 ctrl+alt+delete해서 문제있는 프로세스 제거.
                                         unix는 ps명령어로 현재 프로세스 리스트를 츨력한 뒤,
                                         kill또는 killall명령으로 제거함
